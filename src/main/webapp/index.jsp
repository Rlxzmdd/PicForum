...
<div class="container">
    ...
    <% List<PostInfo> postList = postInfoDao.getPostListByPage(1, 4); %>
    ...
    <div id="post-list">
        <% for (PostInfo post : postList) { %>
            <div class="post-card" style="margin: 10px; padding: 10px;">
                ...
                <p class="post-type"><%=Objects.equals(post.getType(), "show") ? "分享" : "约拍" %>
                </p>
...
<script>
    ...
    // 定义每页显示条数变量，与后台保持一致
    var pageSize = 4;
    // 如果flag为false，就代表之后没有数据要刷新了，停止请求
    var flag = true;
    // 给window绑定滚动事件
    $(window).scroll(function () {
        if (flag)
            ...
            if ($(window).scrollTop() + $(window).height() === $(document).height()) {
                ...
                // List<PostInfo> postList = postInfoDao.getPostListByPage(1, 4);
                $.get("post?action=listByPage", {page: currentPage,size:pageSize, type: type}, function (data) {
                    ...
                    if (data == null || data.length === 0) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            var date = new Date(post.timeUpdate);
                            html += '<p class="post-date">' + date.toLocaleString() + '</p>';
                            //todo 调整时间格式
                            html += '<a  class="lookdetail"  href="post?action=detail&pid=' + post.pid + '">查看详情</a>';
                            html += '</div>';
                        }
                        post_list.append(html);
                        // $(html).insert(loadMoreBtn);
                    }
                }, "json");
            }
    });
